...
// Matching is case-insensitive on the extension. Shared by the local-directory
// Scan() and the push-model upload endpoint so the two ingestion paths never
// drift out of sync on what counts as a valid file.
